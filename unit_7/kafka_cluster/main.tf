...
resource "yandex_vpc_network" "kafka_network" {
  name        = "kafka-network-${var.cluster_name}"
  description = "Network for Kafka cluster ${var.cluster_name}"
  folder_id   = var.folder_id
  labels      = var.labels
}

# Создаём подсети
resource "yandex_vpc_subnet" "kafka_subnets" {
  count          = length(var.zones)
  name           = "kafka-subnet-${var.cluster_name}-${var.zones[count.index]}"
  zone           = var.zones[count.index]
  network_id     = yandex_vpc_network.kafka_network.id
  v4_cidr_blocks = [cidrsubnet("10.10.0.0/16", 8, count.index + 10)]
  folder_id      = var.folder_id
  labels         = var.labels
}
...
resource "yandex_mdb_kafka_cluster" "kafka_cluster" {
  ...
  labels      = var.labels
  network_id  = yandex_vpc_network.kafka_network.id
  subnet_ids  = yandex_vpc_subnet.kafka_subnets[*].id
...
resource "yandex_mdb_kafka_topic" "kafka_topic" {
  ...
  dynamic "topic_config" {
    ...
    content {
      ...
      # The 'preallocate' field has been deprecated, because feature not useful for Yandex Cloud.
    }
  }
...
resource "yandex_mdb_kafka_user" "admin_user" {
  cluster_id = yandex_mdb_kafka_cluster.kafka_cluster.id
  name       = "admin_user"
  password   = "superpuperpass"
  permission {
    topic_name  = "*"
