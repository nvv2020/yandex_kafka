...
class DataPublisher:
    ...
    def _create_producer(self) -> KafkaProducer:
        ...
        if self.config.ssl_enabled:
            # Конфигурация для отправки JSON
            producer_conf = {
            ...
            logger.info(f"Creating Kafka producer with SSL for JSON publishing")
            
            # Проверяем существование файлов
            for file_type, file_path in [
                ("CA", f"{base_path}/ca.crt"),
                ("Certificate", f"{base_path}/client.crt"),
                ("Key", f"{base_path}/client.key")
            ]:
                if os.path.exists(file_path):
                    logger.debug(f"  ✓ {file_type} file exists: {file_path}")
                else:
    ...
    def _check_connection(self) -> bool:
        ...
        try:
            # Простая проверка - пытаемся получить метаданные кластера
            # Не создаем лишних топиков!
