...
async def on_started():
    ...
    logger.info(f"📊 Начальное количество запрещенных слов: {word_count}")
...
def check_text_for_profanity(text: str) -> Dict:
    ...
    """
    # Получаем все запрещенные слова из таблицы
    banned_words = list(banned_words_table.keys())
...
async def process_shop_messages(stream):
    ...
    async for message in stream:
        try:
            ...
            current_banned_words = list(banned_words_table.keys())
...
class StatsAPIView(View):
    async def get(self, request: Request) -> Response:
        ...
        try:
            ...
            banned_words_count = len(banned_words_table)
            
            # Получаем детальную информацию о запрещенных словах
            banned_words_list = []
            for word, info in banned_words_table.items():
...
class AddBannedWordAPIView(View):
    async def post(self, request: Request) -> Response:
        ...
        try:
            ...
            if word_lower in banned_words_table:
                return self.json({
                    'status': 'error',
                    'message': f'Слово "{word}" уже в списке запрещенных'
                }, status=409)
            ...
            # Также добавляем сразу в таблицу для немедленного эффекта
            banned_words_table[word_lower] = {
                'word': word_lower,
                'added_at': update.timestamp,
                'source': update.source,
                'request_id': update.request_id
...
class RemoveBannedWordAPIView(View):
    async def post(self, request: Request) -> Response:
        ...
        try:
            ...
            if word_lower not in banned_words_table:
                return self.json({
                    'status': 'error',
                    'message': f'Слово "{word}" не найдено в списке запрещенных'
                }, status=404)
            ...
            # Также удаляем сразу из таблицы для немедленного эффекта
            banned_words_table.pop(word_lower, None)
        ...
        try:
            ...
            for word in words:
                try:
                    ...
                    # Также добавляем сразу в таблицу
                    banned_words_table[word_lower] = {
                        'word': word_lower,
                        'added_at': update.timestamp,
                        'source': update.source,
...
class ClearBannedWordsAPIView(View):
    async def post(self, request: Request) -> Response:
        ...
        try:
            ...
            # Получаем все слова для удаления
            words_to_remove = list(banned_words_table.keys())
            removed_count = 0
            ...
            for word in words_to_remove:
                try:
                    ...
                    # Также удаляем сразу из таблицы
                    banned_words_table.pop(word, None)
                    removed_count += 1
...
class CheckWordAPIView(View):
    ...
    async def _check_word(self, word: str) -> Response:
        ...
        word_lower = word.lower()
        is_banned = word_lower in banned_words_table
        
        word_info = None
        if is_banned:
            info = banned_words_table[word_lower]
            word_info = info
        ...
        try:
            ...
            return self.json({
                ...
                'data': {
                    ...
                    'total_banned_words': len(banned_words_table),
...
class IndexView(View):
  ...
  -d '{"confirm": true}'</pre>
            ...
            <div class="endpoint">
                ...
                <p>Протестировать фильтрацию текста и тегов</p>
                <div class="example">
                    <strong>Пример:</strong><br>
